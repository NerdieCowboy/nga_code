...
cube_fn = [fn_path 'sub_sc_crop_cal_Pacino_ChristMajesty_vnir_xnir'];
derivative_fn = [fn_path 'slope_sub_sc_crop_cal_Pacino_ChristMajesty_vnir_xnir'];
derivative2_fn = [fn_path 'slope_slope_sub_sc_crop_cal_Pacino_ChristMajesty_vnir_xnir'];
derivative3_fn = [fn_path 'slope_slope_slope_sub_sc_crop_cal_Pacino_ChristMajesty_vnir_xnir'];
hsz = 3;
dthresh = 0.01;
                            ...
                            if (rdist(q) < tmp(k))
                                ...
                                if (~strcmp(tmpBW,'0'))
                                    ...
                                    if (~strcmp(tmpBW1{k},'0'))
                                        %{
                                        tmpBW2 = regexp(tmpBW1{k}, '\:', 'split');
                                        lbBW = str2double(tmpBW2(1));
                                        ubBW = str2double(tmpBW2(2));
                                        mdBW = (ubBW - lbBW)/2 + lbBW;
                                        %}
                                        ...
                                        pk_m = rtmp(q,1);
                                        pk_n = rtmp(q,2);
                                        %cube_tmp = multibandread(cube_fn,[m0,n0,p0],datatype,headeroffset,interleave,byteorder,{'Column','Direct',pk_n},{'Row','Direct',pk_m},{'Band','Range',[pk1 pk2]});
                            ...
                            if (rdist(q) < tmp(k))
                                ...
                                if (~strcmp(tmpBW,'0'))
                                    ...
                                    if (~strcmp(tmpBW1{k},'0'))
                                        ...
                                        pk_m = rtmp(q,1);
                                        pk_n = rtmp(q,2);
                                        %cube_tmp = multibandread(derivative_fn,[m0,n0,p0],datatype,headeroffset,interleave,byteorder,{'Column','Direct',pk_n},{'Row','Direct',pk_m},{'Band','Range',[pk1 pk2]});
    ...
    for n = 1:n0
        for i = 1:Lp
            ...
            if (matc > 0)
                ...
                rat = (2*tot - matc)/tot;
                score(pnt,:) = [m n i rat*sum(features1(ind,6))];
                pnt = pnt + 1;
            end
        end
    end
