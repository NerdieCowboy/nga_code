function [] = library_derivative(fn_path,trial,hsz,firstd)

%{
fn_path = 'C:\data\Pacino\RIS\';
trial = 'slope_slope_resampled_EarlyRennaisance4';
hsz = 3;
firstd = 0;
library_derivative(fn_path,trial,hsz,firstd)
%}
...
for n = (hsz+ceil(hsz/2)):(n0-hsz-floor(hsz/2))
    tmp = multibandread(fn,[m0,n0,p0],datatype,headeroffset,interleave,byteorder,{'Column','Range',[n-hsz-floor(hsz/2) n+hsz+floor(hsz/2)]});
    if (firstd == 1)
        lt = mean(tmp(:,1:hsz,:),2);
        md = mean(tmp(:,(hsz+1):(2*hsz),:),2);
        rt = mean(tmp(:,(2*hsz+1):(3*hsz),:),2);
    else
        lt = tmp(:,ceil(hsz/2),:);
        md = tmp(:,(hsz + ceil(hsz/2)),:);
        rt = tmp(:,(2*hsz + ceil(hsz/2)),:);
    end
    data3(:,n,:) = single(rt - lt);
    clear lt md rt
    ...
    %mxtmpA = max(tmpA(:));
    %tmpA = tmpA/mxtmpA;
    %lt = mean(tmpA(:,1:hsz,:),2);
    %md = mean(tmpA(:,(hsz+1):(2*hsz),:),2);
    %rt = mean(tmpA(:,(2*hsz+1):(3*hsz),:),2);
    %dataA(:,n,:) = single(rt - lt);
...
for n = 1:n0
    multibandwrite(single(data3(:,n,:)),fn2,'bsq',[1,n,1],[m0,n0,p0]);
    %multibandwrite(single(dataA(:,n,:)),fn2A,'bsq',[1,n,1],[m0,n0,p0]);
end
%copyfile(fnh,fn2h);
%copyfile(fnh,fn2Ah);
