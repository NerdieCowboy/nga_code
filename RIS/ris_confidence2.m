...
features_table_fn = 'C:\damon\dissertation\features_pigments_throughRenasaince_test2.csv';
...
fclose(fid);
cube = multibandread(derivative_fn,[m0,n0,p0],datatype,headeroffset,interleave,byteorder);
...
for j = 1:Lf
    ...
    if (sum(ind) > 0)
        ...
        %{
        [h,~] = hist(tmp,1001);
        nout = min(tmp):(max(tmp)-min(tmp))/1000:max(tmp);
...
for i = 1:Lp
    ...
    caxis([0 1])
    colormap(gray)
    fn = [cube_fn '_' spectra_names{i} '.tif'];
