...
%
% Usage: [mosaic] = mosaic_full(fn_path,set_name,trial,fn_out,mmpixel,N,feather_width,pc,ncores,turnonoutput,quick)
%   inputs: fn_path -> path to easel CSV file
%           set_name  -> folder containing subimages
%           trial -> pre-number string of image filenames
%           fn_out -> filename of resulting image (without extension)
%           mmpixel -> mm/pixel
%           N -> largest number after image filename string
...
function [] = mosaic_full(fn_path,set_name,trial,fn_out,mmpixel,N,feather_width,pc,ncores,turnonoutput,quick,usegui)

if (quick == 1)
    mosiac_quick(fn_path,set_name,trial,fn_out,mmpixel,N,pc,turnonoutput,usegui);
else
...
bord = 2*floor(bord/2);
if (pc == 1)
    offset_val = csvread([fn_path '\' set_name '.csv']);
elseif (pc == 0)
    offset_val = csvread([fn_path '/' set_name '.csv']);
end
...
if (pc == 1)
    fn = [fn_path '\' set_name '\' trial num2str(sprintf('%05.0f',N)) '.tif'];
elseif (pc == 0)
    fn = [fn_path '/' set_name '/' trial num2str(sprintf('%05.0f',N)) '.tif'];
end
img = imread(fn,'tif');
...
for i = 1:(N-1)
    imglist(pnt1) = i;
    if (pc == 1)
        fn1 = [fn_path '\' set_name '\' trial num2str(sprintf('%05.0f',i)) '.tif'];
    elseif (pc == 0)
        fn1 = [fn_path '/' set_name '/' trial num2str(sprintf('%05.0f',i)) '.tif'];
    end
    if (exist(fn1,'file') == 2)
        ...
        for j = (i+1):N
            if (pc == 1)
                fn2 = [fn_path '\' set_name '\' trial num2str(sprintf('%05.0f',j)) '.tif'];
            elseif (pc == 0)
                fn2 = [fn_path '/' set_name '/' trial num2str(sprintf('%05.0f',j)) '.tif'];
            end
            if (exist(fn2,'file') == 2)
...
parfor k = 1:cnt
    ...
    %{
    if (turnonoutput==1)
        disp(['Number of feature point pairs: ', num2str(mt)]);
    end
    if (usegui==1)
        strimg = [trial num2str(sprintf('%05.0f',i)) ' <-> ' trial num2str(sprintf('%05.0f',j))];
        strnum = ['Number of feature point pairs: ', num2str(mt)];
        handles = guidata(PaintingStitch);
        set(handles.text18,'String',strnum);
        set(handles.text20,'String',strimg);
...
if (pc == 1)
    fn1 = [fn_path '\' set_name '\' trial num2str(sprintf('%05.0f',imglist(ind))) '.tif'];
elseif (pc == 0)
    fn1 = [fn_path '/' set_name '/' trial num2str(sprintf('%05.0f',imglist(ind))) '.tif'];
end
img1 = imread(fn1,'tif');
...
for i = 1:num
    if (i ~= ind)
        if (pc == 1)
            fn1 = [fn_path '\' set_name '\' trial num2str(sprintf('%05.0f',imglist(i))) '.tif'];
        elseif (pc == 0)
            fn1 = [fn_path '/' set_name '/' trial num2str(sprintf('%05.0f',imglist(i))) '.tif'];
        end
        img1 = imread(fn1,'tif');
        ...
        clear u0 v0
        parfor k = 1:3
            [mosaic0(:,:,k),dist_from_center0(:,:,k)] = apply_mapping(img1(:,:,k),X0,gridsz,Ntran);
        end
        clear img
        m21 = m211 + m1 - 1;
        m22 = m221 + m1 - 1;
        ...
        if (~isempty([A1y A1x]) && ~isempty([B1y B1x]) && ~isempty([Cy Cx]))
            for p = 1:length(Cy)
                feather_regionA(Cy(p),Cx(p)) = dA(p)/(dA(p) + dB(p));
                feather_regionB(Cy(p),Cx(p)) = dB(p)/(dA(p) + dB(p));
            end
        end
        clear kA kB dA dB
        clear Cx Cy A1x A1y B1x B1y new_msk
        ...
        if (usegui == 1)
            ...
            axes(handles.axes1)
            if (bd == 8)
                imshow(uint8(mosaic(1:4:end,1:4:end,:)));
            elseif (bd == 16)
                imshow(uint16(mosaic(1:4:end,1:4:end,:)));
            end
            drawnow;
        end
    end
...
if (turnonoutput == 1)
    if (pc == 1)
        imwrite(mosaic,[fn_path '\' fn_out '.tif'],'tif','Compression','None');
    elseif (pc == 0)
        imwrite(mosaic,[fn_path '/' fn_out '.tif'],'tif','Compression','None');
    end
end
clear mosaic dist_from_center
end
end
...
function [z,zd] = apply_mapping(I2,X,gridsz,Ntran)
%% apply_mapping
...
while (stopm < 2)
    ...
    while (stopn < 2)
        ...
        F = TriScatteredInterp(double(v2),double(u2),double(tmp(:)),'natural');
        ...
        clear z0 zd0 msk msk2
        
        n11 = n12 - round(gridsz/32) + 1;
        n12 = n11 + gridsz - 1;
        if (n12 >= nend)
            n12 = nend;
            stopn = stopn + 1;
...
function [] = mosiac_quick(fn_path,set_name,trial,fn_out,mmpixel,N,pc,turnonoutput,usegui)
