...
% Last Modified by GUIDE v2.5 10-Oct-2014 12:41:58
...
xrf_mode = get(handles.xrf_mode,'Value');
if (scan_mode == 1)
...
%if ((datatype0 == 4) || (datatype0 == 5) || (xrf_mode == 1))
if (xrf_mode == 1)
    mxX = 0;
    mnX = 2^16-1;
    for j = N1:N2
    ...
    if (exist(fn,'file') == 2)
        ...
        for i = 1:p0
            if (pc == 1)
                ...
                tmp = cube(:,:,i);
                tmp = tmp(:);
                fid = fopen([fn_path trial '__' num2str(sprintf('%03.0f',i)) '\' trial '__' num2str(sprintf('%03.0f',i)) '_' num2str(sprintf('%03.0f',j)) '.bin'],'w');
                fwrite(fid,single(m0),'single');
                fwrite(fid,single(n0),'single');
                fwrite(fid,single(tmp),'single');
                fclose(fid);
                clear tmp
            elseif (pc == 0)
                if (xrf_mode == 1)
...
% --- Executes on button press in run_tot.
function run_tot_Callback(hObject, eventdata, handles)
% hObject    handle to run_tot (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.running,'String','Running ...');
drawnow;
...
if (xrf_mode == 1)
    ...
    if ((datatype0 == 4) || (datatype0 == 5))
        ...
        mxX = 2^16-1;
        mnX = 0;
        multibandwrite(single(tmp),fn,'bsq',[1,1,test_band],[m2,n2,p0],'machfmt','ieee-le')
    end
end
...
parfor i = 1:p0
%for i = 1:p0
    fn = [fn_path trial '__' num2str(sprintf('%03.0f',i)) '.bin'];
    %if (xrf_mode == 1)
        timg = apply_transform(fn,blknum,m2,n2,Ntran,init_reg,Xstar,feather_width,1,0);
        fn_full = [fn_path trial '_full'];
        multibandwrite(single(timg),fn_full,'bsq',[1,1,i],[m2,n2,p0],'machfmt','ieee-le')
    
        %{
        else
        %if (i ~= test_band)
        timg = apply_transform(fn,blknum,m2,n2,Ntran,init_reg,Xstar,feather_width,xrf_mode,0);
...
% Hints: get(hObject,'String') returns contents of Npoly as text
%        str2double(get(hObject,'String')) returns contents of Npoly as a double
