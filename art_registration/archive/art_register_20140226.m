...
% Latest Revision: 21 November 2013
%
% Usage: art_register(rgb_fn,sc,ir_fn,j01,j02,repcnt_max,max_shift,minpts,Npoly,Ntran,memory_limited,morepts,useresults,filteron,addsharp,debugmode,usegui)
...
%           useresult -> 1 (use overlap region in point-matching), 0 (do not use overlap)
        ...
        if (i <= p0)
            ...
            if (exist(fn_full2,'file') == 2)
                ...
                if (fail == 0)
                    ...
                    img21 = single(imread(fn_full2,'tif'));
                    %repcnt = 0;
                    [Xstar,pass,img2_reg,overlap_msk,mxd1,mf1] = incremental_registration(image0bin(:,:,p),img2_reg,overlap_msk,img21,max_shift,samp_ratio,init_reg,mxd,j01,j02,turnonoutput,turnonplot,0,filteron,m1x,n1x,p1,minpts,feather_width,gpu,morepts,p,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui);
...
for i = (inc-1):-1:1
    if (length(find(pass_test == i)) == 1)
        ...
        [init_reg00,Xstar00,mult,previ,img2_reg,overlap_msk,mxd1,mf1,boost,color] = reg_loop(image0bin,image1tif,img2_reg,overlap_msk,fn_path,fn_path2,trial,rel_reg,repcnt_max,init_reg00,Xstar00,mult,max_shift,samp_ratio,down,mxd,j01,j02,i,previ,direction,turnonoutput,turnonplot,useresults,pc,filteron,m1x,n1x,minpts,bd2,feather_width,gpu,morepts,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui);
        clear reg_loop
...
for i = (min(pass_test)+1):(inc-1)
    if (length(find(pass_test == i)) == 1)
        ...
        [init_reg00,Xstar00,mult,previ,img2_reg,overlap_msk,mxd1,mf1,boost,color] = reg_loop(image0bin,image1tif,img2_reg,overlap_msk,fn_path,fn_path2,trial,rel_reg,repcnt_max,init_reg00,Xstar00,mult,max_shift,samp_ratio,down,mxd,j01,j02,i,previ,direction,turnonoutput,turnonplot,0,pc,filteron,m1x,n1x,minpts,bd2,feather_width,gpu,morepts,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui);
        clear reg_loop
        reg_results(i,:) = [i mxd1 mf1 color boost];
        fn_full2 = [fn_path2 'registration_results.csv'];
        csvwrite(fn_full2,reg_results)
...
clear rel_reg
clear image0bin image1tif img2_reg overlap_msk
if (memory_limited == 1)
    delete('image1.tif')
    delete('image0.bin')
end
...
function [init_reg00,Xstar00,mult,previ,img2_reg,overlap_msk,mxd1,mf1,boost,color] = reg_loop(image0bin,image1tif,img2_reg,overlap_msk,fn_path,fn_path2,trial,rel_reg,repcnt_max,init_reg00,Xstar00,mult,max_shift0,samp_ratio,down,mxd,j01,j02,i,previ,direction,turnonoutput,turnonplot,useresults,pc,filteron,m2,n2,minpts,bd2,feather_width,gpu,morepts,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui)
%% registration
...
if (exist(fn_full2,'file') == 2)
    ...
    while ((pass == 0) && (repcnt <= repcnt_max))
        ...
        if (fail == 0)
            if (boost == 0)
                [Xstar0,pass,img2_reg,overlap_msk,mxd1,mf1] = incremental_registration(image0bin(:,:,color),img2_reg,overlap_msk,img21,max_shift,samp_ratio,init_reg0,mxd,j01,j02,turnonoutput,turnonplot,useresults,filteron,m2,n2,p1,minpts,feather_width,gpu,morepts,color,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui);
            else
                [Xstar0,pass,img2_reg,overlap_msk,mxd1,mf1] = incremental_registration(image0bin(:,:,color),img2_reg,overlap_msk,img21,max_shift,samp_ratio,init_reg0,mxd,j01,j02,turnonoutput,turnonplot,useresults,filteron,m2,n2,p1,minpts,feather_width,gpu,1,color,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui);
            end
            clear incremental_registration
        else
            mult = mult + 1;
            ...
            if (search_test == 0)
                ...
                if ((p1 > 1) && (color < p1))
                    ...
                    if (boost == 0)
                        %mxd = 1/samp_ratio;
                        color = color + 1   %change color band
                    elseif ((mxd1 < 2/samp_ratio) && (addsharp == 1))
                        %mxd = 2/samp_ratio
                        color = color + 1
                    else
                        %mxd = 1/samp_ratio;
...
function [X,pass,img2_reg,overlap_msk,mxd1,mf1] = incremental_registration(image0bin,img2_reg,overlap_msk,img21,max_shift,samp_ratio,init_reg,mxd,j01,j02,turnonoutput,turnonplot,useresults,filteron,m2,n2,p1,minpts,feather_width,gpu,morepts,color,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui)
%% 

pass = 1;

[X mxd1 mf1 F2] = art_regx(image0bin,img2_reg,overlap_msk,img21,init_reg,max_shift,samp_ratio,mxd,j01,j02,useresults,filteron,m2,n2,p1,minpts,gpu,morepts,color,memory_limited,Npoly,Ntran,turnonplot,addsharp,debugmode,usegui);
clear art_regx
if (usegui ~= 0)
...
if (mxd1 <= mxd)
    ...
    [tmp,~,~] = apply_mapping(ones(mimg21,nimg21),init_reg,X,Ntran,m2,n2);
    mskimg21 = (tmp == 1);
    clear tmp
    se = strel('disk',3);
    ...
    % mosaicing
    %[m1,n1] = size(img21);
    [img2_reg,overlap_msk] = mosaic(img2_reg,overlap_msk,img20,m21,n21,feather_width);
    clear mosaic
    clear img20
...
function [X,mxd1,mf1,F2] = art_regx(image0bin,img2_reg,overlap_msk,img1,init_reg,max_shift,samp_ratio,mxd,j01,j02,useresults,filteron,m2,n2,p1,minpts,gpu,morepts,color,memory_limited,Npoly,Ntran,turnonplot,addsharp,debugmode,usegui)
...
function [img2_reg,overlap_msk] = mosaic(img2_reg,overlap_msk,img20,m21,n21,feather_width)
%% mosaicing
...
%{
blk = 16;
temp1 = img2_reg.*intersect_msk;
temp2 = img20.*intersect_msk;
rgb_fn = [fn_path trial '_RGB.tif'];
rgb = imread(rgb_fn,'tif','PixelRegion',{[m21,m22],[n21,n22]});
ref = rgb2gray(rgb);
clear rgb
mi1 = zeros((m22-m21+1),(n22-n21+1));
mi2 = zeros((m22-m21+1),(n22-n21+1));
%}
se = strel('square',3);
