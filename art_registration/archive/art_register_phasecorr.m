function [] = art_register(rgb_fn,sc,ir_fn,j01,j02,repcnt_max,max_shift,minpts,Npoly,Ntran,memory_limited,morepts,filteron,addsharp,debugmode,usegui,mxd)
...
%
% Usage: 
%   inputs: rgb_fn -> filename of reference image
%           sc -> scale factor for reference image
%           ir_fn -> filename for collection of template images
...
%           Npoly -> order of the polynomial fit (0 -> bilinear)
%           Ntran -> order of the polynomial transformation (0 -> bilinear)
%           memory_limited -> peak memory will fit in RAM -> 0, otherwise -> 1
%           morepts -> force maximum number of feature points to be extracted -> 1, otherwise -> 0
%           filteron -> 1 (median filter  on), 0 (median filter off)
%           addsharp -> 1 (use for difficult images), 0 (use for all other images)
%           debugmode -> 1 (display feature points & disparity plane), 0 (do not display)
...
%% *********************************************
% RGB -> R
% read/create reference image (image0tif) and scaled reference image
%   (image1tif)
info = imfinfo(rgb_fn);
    ...
    while (pass == 0)
        if (i <= p0)
            ...
            if (exist(fn_full2,'file') == 2)
                ...
                clear img21
                [cctable_out fail] = init_pass(img1,img2,img1_corner,addsharp);
                clear img2
                clear init_pass
                ...
                if (fail == 0)
                    ...
                    img21 = single(imread(fn_full2,'tif'));
                    %repcnt = 0;
                    [Xstar,pass,img2_reg,zd,blknum,mxd1,mf1] = incremental_registration(image0tif(:,:,p),img2_reg,zd,blknum,tifnumlist(IX(i)),img21,max_shift,samp_ratio,init_reg,mxd,j01,j02,turnonoutput,turnonplot,filteron,m1x,n1x,p1,minpts,morepts,p,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui);
...
for i = (inc-1):-1:1
    if (length(find(pass_test == i)) == 1)
        ...
        [init_reg00,Xstar00,mult,previ,img2_reg,zd,blknum,mxd1,mf1,boost,color] = reg_loop(image0tif,image1tif,img2_reg,zd,blknum,fn_path,fn_path2,trial,rel_reg,repcnt_max,init_reg00,Xstar00,mult,max_shift,samp_ratio,down,mxd,j01,j02,i,previ,direction,turnonoutput,turnonplot,pc,filteron,m1x,n1x,minpts,bd2,morepts,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui);
        clear reg_loop
...
for i = (min(pass_test)+1):(inc-1)
    if (length(find(pass_test == i)) == 1)
        ...
        [init_reg00,Xstar00,mult,previ,img2_reg,zd,blknum,mxd1,mf1,boost,color] = reg_loop(image0tif,image1tif,img2_reg,zd,blknum,fn_path,fn_path2,trial,rel_reg,repcnt_max,init_reg00,Xstar00,mult,max_shift,samp_ratio,down,mxd,j01,j02,i,previ,direction,turnonoutput,turnonplot,0,pc,filteron,m1x,n1x,minpts,bd2,morepts,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui);
        clear reg_loop
        reg_results(i,:) = [i mxd1 mf1 color boost];
        fn_full2 = [fn_path2 'registration_results.csv'];
        csvwrite(fn_full2,reg_results)
...
clear rel_reg
clear image1tif img2_reg zd blknum
if (memory_limited == 1)
    delete('image1.tif')
end
end

function [init_reg00,Xstar00,mult,previ,img2_reg,zd,blknum,mxd1,mf1,boost,color] = reg_loop(image0tif,image1tif,img2_reg,zd,blknum,fn_path,fn_path2,trial,rel_reg,repcnt_max,init_reg00,Xstar00,mult,max_shift0,samp_ratio,down,mxd,j01,j02,i,previ,direction,turnonoutput,turnonplot,pc,filteron,m2,n2,minpts,bd2,morepts,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui)
%% registration
...
if (exist(fn_full2,'file') == 2)
    ...
    while ((pass == 0) && (repcnt <= repcnt_max))
        ...
        if (fail == 0)
            if (boost == 0)
                [Xstar0,pass,img2_reg,zd,blknum,mxd1,mf1] = incremental_registration(image0tif(:,:,color),img2_reg,zd,blknum,i,img21,max_shift,samp_ratio,init_reg0,mxd,j01,j02,turnonoutput,turnonplot,filteron,m2,n2,p1,minpts,morepts,color,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui);
            else
                [Xstar0,pass,img2_reg,zd,blknum,mxd1,mf1] = incremental_registration(image0tif(:,:,color),img2_reg,zd,blknum,i,img21,max_shift,samp_ratio,init_reg0,mxd,j01,j02,turnonoutput,turnonplot,filteron,m2,n2,p1,minpts,1,color,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui);
            end
            clear incremental_registration
        else
            mult = mult + 1;
...
function [cctable_out,fail] = init_pass(img1,img2,img1_corner,addsharp)
%% *********************************************
[m1sub n1sub p1sub] = size(img1);
[m2sub n2sub] = size(img2);
mtsub = m1sub + m2sub - 1;
ntsub = n1sub + n2sub - 1;

img21 = single(fft2((img2-mean2(img2)),mtsub,ntsub));
img211 = angle(img21);
clear img21
...
for p = 1:p1sub
    img11 = single(fft2((img1(:,:,p)-muimg1),mtsub,ntsub));
    img111 = angle(img11);
    clear img11
    
    if (max(max(img2(:))) ~= min(img2(:)) && (m1sub>=m2sub) && (n1sub>=n2sub))
        cc = ifft2(exp(1i*(img111-img211)));
    else
        cc = 0;
    end
    clear img111
...
clear img1 img2 img1_corner
end

function [X,pass,img2_reg,zd,blknum,mxd1,mf1] = incremental_registration(image0tif,img2_reg,zd,blknum,blknum0,img21,max_shift,samp_ratio,init_reg,mxd,j01,j02,turnonoutput,turnonplot,filteron,m2,n2,p1,minpts,morepts,color,memory_limited,Npoly,Ntran,addsharp,debugmode,usegui)
%% 

pass = 1;

[X mxd1 mf1 F2] = art_regx(image0tif,img21,init_reg,max_shift,samp_ratio,mxd,j01,j02,filteron,m2,n2,minpts,morepts,memory_limited,Npoly,Ntran,turnonplot,addsharp,debugmode,usegui);
clear art_regx
if (usegui ~= 0)
...
if (mxd1 == 0)
    ...
    [tmp,~,~] = apply_mapping1(2^15*ones(mimg21,nimg21),init_reg,X,Ntran,m2,n2);
    ...
    clear u1 v1
    [zd1,~,~] = apply_mapping1(zd0,init_reg,X,Ntran,m2,n2);
    zd1(mskimg21 == 0) = sqrt(mimg21^2 + nimg21^2);
    zd1 = uint16(zd1);
    clear mskimg21 zd0
    %figure(101),imagesc(zd1)
    %pause
    clear apply_mapping apply_mapping1
    [m0 n0] = size(img20);
...
function [X,mxd1,mf1,F2] = art_regx(image0tif,img1,init_reg,max_shift,samp_ratio,mxd,j01,j02,filteron,m2,n2,minpts,morepts,memory_limited,Npoly,Ntran,turnonplot,addsharp,debugmode,usegui)
...
img11(init_reg(1):(init_reg(1)+m1-1),init_reg(2):(init_reg(2)+n1-1)) = img111;
...
for i = mf1:-1:1
    ...
    if ((F1(i,1) < 1) || (F1(i,2) < 1) || (F1(i,1) > n2) || (F1(i,2) > m2))
        ...
        tmp1 = single(imresize(tmp,samp_ratio,'bicubic'));
        clear tmp
        ...
        % read data from disk
        if (memory_limited == 1)
            tmp = imread('image0.tif','PixelRegion',{[tp bt],[lt rt]});
            muimg21 = mean(tmp(:));
        else
            tmp = image0tif(tp:bt,lt:rt);
            muimg21 = mean(image0tif(:));
        end
        % need to update to work with new code 20140306
        tmp2 = single(imresize(tmp,samp_ratio,'bicubic'));
        clear tmp
        d = floor(samp_ratio/2)/samp_ratio;
        ...
        mv1 = mt1 + mt2 - 1;
        nv1 = nt1 + nt2 - 1;
        F11 = fft2((tmp1-muimg11),mv1,nv1);
        P11 = angle(F11);
        F21 = fft2((tmp2-muimg21),mv1,nv1);
        P21 = angle(F21);
        cc = abs(ifft2(exp(1i*(P21-P11))));        
        
        clear tmp F11 P11 F21 P21
...
function [X] = mapping(F11,F21,Ntran)
%% mapping
...
%{
[v1,u1] = meshgrid(n21:n22,m21:m22);
z = interp2(double(v2(:)),double(u2(:)),double(I2(:)),v1,u1);
clear u2 v2 I2
%}
z(isnan(z)) = 0;
% removed 20140306
...
function [z,m21,n21] = apply_mapping1(I2,init_reg,X,Ntran,mfull,nfull)
%% apply_mapping
% Transform second image so that it is registered with the first
%   Input: I2 -> image
%          X -> transform coefficients
...
F = TriScatteredInterp(double(v2(:)),double(u2(:)),double(I2(:)),'nearest');
clear u2 v2 I2
[v1,u1] = meshgrid(n21:n22,m21:m22);
z = F(v1,u1);
z(isnan(z)) = 0;
