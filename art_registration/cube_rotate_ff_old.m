...
function [] = cube_rotate_ff(ir_fn,w_fn,usegui)
...
[m0,n0,p0] = size(cube);
multibandwrite(uint16(cube),[fn_path trial],'bsq','machfmt','ieee-le')
...
if (usegui == 1)
    handles = guidata(FlatFieldCube);
    set(handles.axes1,'Visible','on');
    axes(handles.axes1)
    imshow(cube(:,:,round(p0/2)));
