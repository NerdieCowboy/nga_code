...
cube = single(multibandread(trial_out,[m0,n0,p0],'single',0,'bsq','ieee-le',{'Band','Direct',p1}));
...
num_cols = str2num(get(handles.n0,'String')); %#ok<ST2NM>
num_rows = str2num(get(handles.m0,'String')); %#ok<ST2NM>
num_bands = str2num(get(handles.p0,'String')); %#ok<ST2NM>
...
elseif (read_only == 0)
    ...
    if (exist(fncsv,'file') == 2)
        ...
        dir = 0;
        cnt = 1;
        cube = single(zeros(num_rows,num_cols,num_bands));
        X = single(zeros(m0,n0));
        Y = single(zeros(m0,n0));
        for m = 1:num_rows
            for n = 1:num_cols
                tmp = single(fread(fid,num_bands,'uint32',0,'ieee-le'));
        ...
        dir = 0;
        cube1 = single(zeros(num_rows,num_cols,num_bands));
        for m = 1:num_rows
            for n = 1:num_cols
                if (dir == 0)
                    if (n > 1)
                        cube1(m,n,:) = cube(m,n,:) - cube(m,(n-1),:);
                    else
                        cube1(m,n,:) = cube(m,2,:) - cube(m,1,:);
                    end
                elseif (dir == 1)
                    n1 = num_cols - (n - 1);
                    if (n1 < num_cols)
                        cube1(m,n1,:) = cube(m,n1,:) - cube(m,(n1+1),:);
                    else
                        cube1(m,n1,:) = cube(m,(num_cols-1),:) - cube(m,num_cols,:);
                    end
                end
            end
            if (dir == 0)
        ...
        dir = 0;
        cube2 = single(zeros(num_rows,num_cols,num_bands));
        for m = 1:num_rows
            for n = 1:num_cols
                if (dir == 0)
                    if (n > 1)
                        cube2(m,n,:) = cube(m,n,:) - cube(m,(n-1),:);
                    else
                        cube2(m,n,:) = cube(m,2,:) - cube(m,1,:);
                    end
                elseif (dir == 1)
                    n1 = num_cols - (n - 1);
                    if (n1 < num_cols)
                        cube2(m,n1,:) = cube(m,n1,:) - cube(m,(n1+1),:);
                    else
                        cube2(m,n1,:) = cube(m,(num_cols-1),:) - cube(m,num_cols,:);
                    end
                end
            end
            if (dir == 0)
...
energy1 = p1*slope+yint;
set(handles.energy1,'string',num2str(energy1));
tmp= single(multibandread(trial_out,[m0,n0,p0],'single',0,'bsq','ieee-le',{'Band','Direct',p1}));
imagesc(single(tmp))
colormap gray
%axis image
axis off
...
if ((x>=1) && (x<=pos(3)) && (y>=1) && (y<=pos(4)))
    ...
    trial_out = [img_path trial];
    cube = single(multibandread(trial_out,[m0,n0,p0],'single',0,'bsq','ieee-le',{'Row','Range',[y,y]},{'Column','Range',[x,x]},{'Band','Range',[p3,p2]}));
    axes(handles.axes2)
    if (logon == 1)
